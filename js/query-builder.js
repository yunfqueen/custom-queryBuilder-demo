...
var QueryBuilder = function($el, options) {
    ...
    if(this.filters.select){
        this.tables=this.filters.select
    }else{
...
QueryBuilder.selectors = {
    ...
    change_custom_select: '[data-change=custom]'
};
...
QueryBuilder.prototype.bindEvents = function() {
    ...
    // custom select change yun add
    this.$el.on('change.queryBuilder', Selectors.change_custom_select, function() {
        var $rule = $(this).closest('.table-filter');
        var $rule_id = $(this).closest(Selectors.rule_container)[0].id;
        self.loadFilterCombo($rule,$rule_id);
    });
...
QueryBuilder.prototype.addRule = function(parent, data, flags) {
    ...
    //yun add
    var $tableSelect     = $(this.getTableSelect(rule_id));
    //***
...
QueryBuilder.prototype.updateRuleFilter = function(rule, previousFilter) {
        ...
        if(rule.filter&&rule.filter.id){
            ...
            h+= '</select>';
            rule.$el.find(QueryBuilder.selectors.filter_container).html(h);
            rule.$el.find('.table-filter').val(select_id);
            if(rule.filter.input&&rule.filter.input=='select'){
                var s = '<option value="-1">'+ this.settings.select_placeholder +'</option>';
                $.each(rule.filter.values, function(i, filter) {
                    s+= '<option value="'+ i +'">'+ filter +'</option>';
                });
                rule.$el.find(QueryBuilder.selectors.rule_value).html(s);
            }
        }
        //***
...
{{ var optgroup = null; }} \
<select class="form-control" name="{{= it.rule.id }}_filter" id="{{= it.rule.id }}_filter"> \
...
QueryBuilder.prototype.getTableSelect = function(rule_id) {
    var self = this;
    var h = '<div class="overall-filter-container"><select data-live-search="true" class="table-filter selectpicker btn-sm form-control" data-change="custom" name="table-filter">';
...
QueryBuilder.prototype.loadFilterCombo = function (rule,rule_id) {
    var tableName = $(rule).val();
    var filters_data = this.settings.filters;
    var columns = filters_data[tableName];
    this.loadTableColumnsInFilter(columns, rule_id, tableName)
}
QueryBuilder.prototype.loadTableColumnsInFilter = function(columns, rule_id, tableName) {
...
QueryBuilder.prototype.getExpressionFiltersData = function(column, tableName){
    QueryBuilder.DEFAULTS.filters.push(column);
}
...
QueryBuilder.regional['en'] = {
  ...
  "delete_rule": "Delete",
  "delete_group": "Delete",
  "change_custom_select":"Change Custom Select",
